...
<body>
    ...
    <s:form method="POST" action="bookAction_findBookByNoStatus2">
        ...
        <s:iterator value="#request.bookList" var="b" status="ids">
            <tr>
                ...
                <td>
                    ...
                    <a href="javascript:;" onclick="returnBook(this)">归还</a>
                    &nbsp;&nbsp;
                </td>
            </tr>
        </s:iterator>
    </table>
<a href="javascript:;" onclick="returnAllBook()">出租所选书籍</a>
</body>
<script type="text/javascript">
    ...
    function returnAllBook(){
        ...
        if (ids.length==0){
            ...
            if(confirm('你确定要出租这些书籍吗？')){
                location.href='${pageContext.request.contextPath}/bookAction_returnAllBook?'+param;
                return true;
            }else {
                return false;
            }
    ...
    function returnBook(ob) {
        var $val=$(ob).parent().find("input");
        var theValue= $($val.get(0)).val();
        var $text=$(ob).parent().parent().find("td");
        var theText=$($text.get(3)).text();
        if (confirm(theText+'即将会被归还')){
            location.href="${pageContext.request.contextPath}/bookAction_returnBook?book.bid="+theValue;
            return true;
        }
    }
</script>
